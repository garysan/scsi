...
my $usuario = &validate;
my $reporte = "adminsquidguard.pdf";
my $titulo="Lista de excepciones en SquidGuard";
...
my $page_num  = 1;
my $data =[];
my $sth = $dbh->prepare("SELECT * FROM adlog ") or &dberror;
$sth->execute() or &dberror;

push(@$data , ["Nro","Direccion IP permitida en SquidGuard"]);
